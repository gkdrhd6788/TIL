...
M = int(input()) # 학생 수 100 이하
for _ in range(M):
    num,gender = map(int,input().split()) # 받은 값, 성별
    if gender ==1: # 남학생
        for i in range(1,101): # 1,2,3,4,5 배수
            real_index =num*i - 1
            if real_index < N:
        ...
        for j in range(num-1,0,-1): # num보다 작은 값 기준 역순
            real_index = j-1
            sym_real_index = (num-1)*2-real_index
            if real_index<0 or sym_real_index >= N:
                break
            if switch_status[real_index] == switch_status[sym_real_index]:
                if switch_status[real_index] == 0:
                    switch_status[real_index] = 1
                    switch_status[sym_real_index]=1

                else:
                    switch_status[real_index] = 0
                    switch_status[sym_real_index]=0
            else:
                break

for i in range(0,len(switch_status),20): # 1,21,31
