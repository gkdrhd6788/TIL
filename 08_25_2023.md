...
center = N//2 # 2
total = 0
for i in range(center+1): #0,1,2 중간행까지
    value = arr[i][center]
    total += value
    for k in range(1,i+1): #0에서 i까지 (1에서 1까지는 실행 안됨)
        ii = center + (1)*k
        total += arr[i][ii]
        ii = center +(-1)*k
        total += arr[i][ii]
for j in range(N-1,center,-1): #N-1에서 중간행 전까지(center+1)
    value = arr[i][center]
    total += value
    for k in range(1,N-j): #0에서 i까지 (1에서 1까지는 실행 안됨)
        ii = center + (1)*k
        total += arr[i][ii]
        ii = center +(-1)*k
        total += arr[i][ii]
