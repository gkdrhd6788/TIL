...
다음주 월요일 : 과목평가 객관식 단답서술

그 다음주 월요일 : 월말평가(어려움, 코딩)
...
* 문제 풀때 많이 푼다.  인덱스로 리스트 순회 : range ( len ( numbers ) ),  numbers[ i ]
  - nxn에서 원하는 영역만 읽어올 때
...
* 계속읽다가 중간에 건너뛰고 읽고싶으면 while문 사용 가능 (for 문은 못씀)
...
주의! 가독성을 떨어짐--> 실제 코드 작성할 때는 원래 방법으로 작성하라(이해만 하면 됨)
